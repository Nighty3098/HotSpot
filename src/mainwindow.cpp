    ...
    , ui(new Ui::MainWindow)
{
    ...
    connect(ui->appsList, &QListWidget::itemClicked, this, &MainWindow::onItemClicked);
...
void MainWindow::setupThemeSystem()
{
    ...
    QString savedTheme = globalSettings->value("theme/current", "gruvbox_dark").toString();
...
void MainWindow::loadAvailableThemes()
{
    ...
    QStringList themeFiles = resourceDir.entryList(QStringList() << "*.qss", QDir::Files);
    ...
    QDir fsDir("themes/");
    if (fsDir.exists()) {
        QStringList fsThemeFiles = fsDir.entryList(QStringList() << "*.qss", QDir::Files);
...
void MainWindow::applyTheme(const QString &themeName)
{
    ...
    if (file.open(QFile::ReadOnly)) {
        ...
        qDebug() << "Failed to load theme file:" << selectedTheme.path << file.errorString();
    }
}
...
void MainWindow::showThemeSelection()
{
    ...
    QString currentTheme = globalSettings->value("theme/current", "gruvbox_dark").toString();
...
void MainWindow::launchApp(QListWidgetItem *item) {
    ...
    QString execCommand = item->data(Qt::UserRole + 1).toString();
    if (execCommand.isEmpty() || execCommand == "theme_selector") return;
